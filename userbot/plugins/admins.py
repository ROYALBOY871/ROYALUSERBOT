...
async def promote(promt):
    ...
    event = await edit_or_reply(promt, "Promoting...")
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    event = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
...
async def ban(bon):
    ...
    if not user:
        return
    event = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await RYevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await event.edit(f"{str(user.id)} ιѕ ϐαииє∂ !!\nяєαѕοи: {reason}")
    else:
        await event.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    RYevent = await edit_or_reply(unbon, "Unbanning...")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    RYevent = await edit_or_reply(usr, "Kicking...")
    try:
