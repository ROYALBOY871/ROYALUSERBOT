    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            buttons = [
                ...
                (Button.url(" 🔱Support🔱 ", "https://t.me/ROYALUBOT_SUPPORT"),),
            ]
    else:
        ...
        buttons = [
            (
                Button.url(" Support ", "https://t.me/ROYALUBOT_SUPPORT"),
                Button.url(" Updates ", "https://t.me/ROYALYSERBOT"),
            ),
            (
...
@royal.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rules")))
...
@royal.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"users")))
...
@royal.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
...
async def send_flood_alert(user_) -> None:
    ...
    if found:
        ...
        if not fa_id:
            return
        try:
            msg_ = await royal.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await royal.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await royal.tgbot.get_entity(BOTLOG_CHATID)
            await royal.tgbot.send_message(
                Config.OWNER_ID,
                f"⚠️  **[Bot Flood Warning !](https://t.me/c/{chat.id}/{fa_msg.id})**",
            )
        except UserIsBlockedError:
            if BOTLOG:
                await royal.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@royal.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
...
@royal.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@toyal.bot_cmd(incoming=True, func=lambda e: e.is_private)
@royal.bot_cmd(edited=True, func=lambda e: e.is_private)
